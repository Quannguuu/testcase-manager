...
window.testCaseManagerData.addTestCase = function (testCase) {
  const newId =
    testCasesData.length > 0
      ? Math.max(...testCasesData.map((tc) => tc.id)) + 1
      : 1;
  const newTestCase = {
    ...testCase,
    id: newId,
...
window.testCaseManagerData.getPriorityData = function () {
  ...
  Object.keys(priorities).forEach((priority) => {
    priorities[priority].percentage =
      total > 0 ? Math.round((priorities[priority].count / total) * 100) : 0;
  });
...
window.testCaseManagerData.exportSelectedTestCases = function (
  format = "json"
) {
